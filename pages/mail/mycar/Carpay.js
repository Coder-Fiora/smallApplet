...
Page({
    ...
    onLoad(options) {
        ...
        var uid=token.uid,tid='',cid='',allprice=0;
        ...
        dataArr.map(i=>{
            ...
            var price=(i.price * i.number).toFixed(2);
            allprice+=price*1;
        })
        this.setData({
            cid:cid,
    ...
    addOrder(orderid){
        ...
        var arr=alldata.dataArr;
        var cartList=[];
        var address=alldata.rewardAddress;
        arr.map(i=>{
            var aid=address?address.aid:'';
            var string=i.tid+','+i.cid+','+i.sid+','+aid+','+i.price+','+i.number+','+i.time;
            cartList.push(string)
        })
        var list=cartList.join('@');
        return new Promise(resolve=>{      
            http.saveMallCarOrd({
                data: {
                    ...
                    oid:orderid,
                    type:2,
                    pname:alldata.dataArr[0].pname,
                    cmid:alldata.dataArr[0].cid,
                    amount:alldata.allprice,
    ...
    wx.cloud.callFunction({
      ...
      data: {
        goodName: that.data.dataArr[0].gname,   // 商品名称 或 商品描述
